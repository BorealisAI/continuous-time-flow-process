...
def save_model(
        args, aug_model, encoder, optimizer, epoch, itr, path, encoder_optimizer=None
):
    """
    save latent CTFP model's checkpoint during training

    Parameters:
        args: the arguments from parse_arguments in ctfp_tools
        aug_model: the CTFP Model as the decoder
        encoder: the ode-rnn model as the encoder
        optimizer: optimizer of the latent CTFP model
                   all the encoder and decoder parameters
        epoch: training epoch
        itr: training iteration
        path: path to save the model
        encoder_optimizer: the optimizer for the encoder, used in aggressive training
    """
    torch.save(
        {
            "args": args,
...
if __name__ == "__main__":
    ...
    # optimizer
    parameter_list = list(aug_model.parameters()) + list(encoder.parameters())
    optimizer, num_params = optimizer_factory(args, parameter_list)

    if args.aggressive:
        encoder_optimizer, enc_num_params = optimizer_factory(
            args, encoder.parameters()
        )
    else:
    ...
    if args.resume is not None:
        ...
        if (
                args.aggressive
                and "enc_optim_state_dict" in checkpt.keys()
                and checkpt["enc_optim_state_dict"] is not None
        ):
            encoder_optimizer.load_state_dict(checkpt["enc_optim_state_dict"])
    ...
    for epoch in range(args.begin_epoch, args.num_epochs + 1):
        ...
        for temp_idx, x in enumerate(train_loader):
            ...
            loss, loss_training = run_model(
                args, encoder, aug_model, values, times, vars, masks
            )
            ...
            if args.aggressive and (itr == 0 or itr % args.decoder_frequency != 0):
